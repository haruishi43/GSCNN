...
def save_code(exp_path, date_str):
    ...
    cmd = (
        "tar -czvf {zip_outfile} --exclude='*.pyc' --exclude='*.png' "
...
def evaluate_eval(
    ...
    if mean_iu > args.best_record["mean_iu"]:
        ...
        if args.best_record["epoch"] != -1:
            ...
            assert os.path.exists(best_snapshot), "cant find old snapshot {}".format(
                best_snapshot
            )
    ...
    logging.info("-" * 107)
    fmt_str = (
        "[epoch %d], [val loss %.5f], [mask f1 %.5f], [acc %.5f], [acc_cls %.5f], "
        + "[mean_iu %.5f], [fwavacc %.5f]"
    )
    logging.info(
        fmt_str % (epoch, val_loss.avg, mf_score.avg, acc, acc_cls, mean_iu, fwavacc)
    )
    fmt_str = (
        "best record: [val loss %.5f], [mask f1 %.5f], [acc %.5f], [acc_cls %.5f], "
...
def print_evaluate_results(hist, iu, writer=None, epoch=0, dataset=None):
    ...
    for idx, i in enumerate(iu):
        ...
        fp = "{:5.2f}".format(iu_false_positive[idx] / iu_true_positive[idx])
        ...
        writer.add_scalar(
            "val_class_precision/{}".format(id2cat[idx]),
            iu_true_positive[idx] / (iu_true_positive[idx] + iu_false_positive[idx]),
            epoch,
        )
        writer.add_scalar(
            "val_class_recall/{}".format(id2cat[idx]),
            iu_true_positive[idx] / (iu_true_positive[idx] + iu_false_negative[idx]),
            epoch,
        )
