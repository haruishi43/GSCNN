...
def add_items(
    items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip
):
    ...
    for c in cities:
        ...
        for it in c_items:
            item = (
                os.path.join(img_path, c, it + "_leftImg8bit.png"),
    ...
    if quality == "fine":
        ...
        for mode in modes:
            if mode == "test":
                ...
                logging.info(
                    "{} fine cities: ".format(mode) + str(cv_splits[cv_split][mode])
                )

                add_items(
                    items,
                    aug_items,
                    cv_splits[cv_split][mode],
                    img_path,
                    mask_path,
                    mask_postfix,
                    mode,
...
class CityScapes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.eval_mode:
            return (
                self._eval_get_item(img, mask_copy, self.eval_scales, self.eval_flip),
                img_name,
            )
